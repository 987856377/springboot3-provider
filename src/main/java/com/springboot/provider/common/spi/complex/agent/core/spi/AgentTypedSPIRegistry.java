...
package com.springboot.provider.common.spi.complex.agent.core.spi;

import com.springboot.provider.common.spi.complex.agent.api.spi.type.AgentTypedSPI;
import com.springboot.provider.common.spi.complex.agent.core.exception.AgentServiceProviderNotFoundException;
...
public final class AgentTypedSPIRegistry {
    ...
    public static <T extends AgentTypedSPI> T getRegisteredService(final Class<T> typedSPIClass, final String type) {
        Optional<T> serviceInstance = AgentServiceLoader.getServiceLoader(typedSPIClass)
                .newServiceInstances()
                .stream().filter(each -> each.getType().equalsIgnoreCase(type)).findFirst();
    ...
    public static <T extends AgentTypedSPI> Optional<T> getRegisteredServiceOptional(final Class<T> typedSPIClass, final String type) {
        return AgentServiceLoader.getServiceLoader(typedSPIClass)
                .newServiceInstances()
                .stream().filter(each -> each.getType().equalsIgnoreCase(type)).findFirst();
    }
